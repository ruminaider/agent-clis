...
- **Pi subagent children work again on pi-subagents v0.65.0 and later.** That release began hosting child agents as in-process pi sessions built from typed runtime data, so the host process sets `PI_SUBAGENT_CHILD=1` but exports no `PI_SUBAGENT_RUN_ID`, `PI_SUBAGENT_CHILD_INDEX`, or `PI_SUBAGENT_CHILD_AGENT`. The bootstrap read that as a misconfigured child and refused, which blocked every bash, edit, and execution tool call in every subagent, including read-only ones. Such a child now takes its identity from its own session id: `<parent_task>/pi-subagent/session-<hash>` with agent `agent:pi:subagent:session:<hash>` under the new `subagent-session` source, keeping the real parent link, the harness-derived marker, and `AGENT_LEDGER_AUTO_ASSIGNED=0`, and recording `run_tuple_missing` and `missing_run_env` for review. Only a caller-supplied `--session-id` is accepted, because the ambient `PI_SESSION_ID` names the host session and every sibling child would collide on it; the pi adapter therefore bootstraps these children on their first enforcement-bound tool call rather than eagerly at extension load. A child with no parent context and no run tuple takes the equivalent `auto/pi-subagent/session-<hash>` orphan task. Verify treats `pi-subagent-session-bootstrap` like `pi-subagent-bootstrap`, so `AUTO_ASSIGNED_TASK` stays quiet for a healthy child. A missing run tuple with no session id still fails closed.
