...
def _pipeline_priority(workflows: list[WorkflowSummary]) -> tuple[int, int, int]:
...
def build_pipeline_summary(client: CircleCIClient, project: ResolvedProject, *, pipeline_id: str | None = None) -> PipelineSummary:
    ...
    ranked_candidates: list[tuple[tuple[int, int, int, int], dict[str, Any], list[WorkflowSummary]]] = []

    for index, candidate in enumerate(candidate_pipelines):
        ...
        for workflow in workflows:
            ...
            matching_jobs = [_job_summary(job, project.project_slug) for job in jobs if _job_matches(job, target_config)]
        ...
        if candidate_summaries:
            priority = _pipeline_priority(candidate_summaries)
            ranked_candidates.append((priority + (-index,), candidate, candidate_summaries))
...
def _primary_job(summary: PipelineSummary, status_name: str) -> JobSummary | None:
...
def _pipeline_links(summary: PipelineSummary) -> dict[str, Any]:
    ...
    failed_job = _primary_job(summary, "failed")
    running_job = _primary_job(summary, "running")
    primary_job = failed_job or running_job
...
def summarize_pipeline(summary: PipelineSummary, *, command: str, target: str | None) -> CommandResponse:
    ...
    if failed_job is not None:
        headline = f"Pipeline {summary.number or summary.id} failed in {failed_job.name}"
    elif running_job is not None:
        headline = f"Pipeline {summary.number or summary.id} is still running in {running_job.name}"
